...
/*Correct version, the center of box is  x/2, y/x,z/2*/
void drawBox(double x, double y, double z)
{
	...
	else
	{
		...
		glNormal3d(0.0, 0.0, -1.0); //back
		glVertex3d(-0.5, -0.5f, -0.5f); glVertex3d(-0.5, 0.5, -0.5);
		glVertex3d(0.5, 0.5, -0.5); glVertex3d(0.5, -0.5, -0.5);

		glNormal3d(0.0, -1.0, 0.0); //bottom
		glVertex3d(-0.5, -0.5f, -0.5f); glVertex3d(0.5, -0.5, -0.5);
		glVertex3d(0.5, -0.5, 0.5); glVertex3d(-0.5, -0.5, 0.5);

		glNormal3d(-1.0, 0.0, 0.0);//left
		glVertex3d(-0.5, -0.5f, -0.5f); glVertex3d(-0.5, -0.5, 0.5);
		glVertex3d(-0.5, 0.5, 0.5); glVertex3d(-0.5, 0.5, -0.5);

		glNormal3d(0.0, 0.0, 1.0);//front
		glVertex3d(-0.5, -0.5f, 0.5f); glVertex3d(0.5, -0.5, 0.5);
		glVertex3d(0.5, 0.5, 0.5); glVertex3d(-0.5, 0.5, 0.5);

		glNormal3d(0.0, 1.0, 0.0); //top
		glVertex3d(-0.5, 0.5, -0.5); glVertex3d(-0.5, 0.5, 0.5);
		glVertex3d(0.5, 0.5, 0.5); glVertex3d(0.5, 0.5, -0.5);

		glNormal3d(1.0, 0.0, 0.0);//right
		glVertex3d(0.5, -0.5, -0.5); glVertex3d(0.5, 0.5, -0.5);
		glVertex3d(1.0 - shift, 1.0 - shift, 1.0 - shift); glVertex3d(1.0 - shift, 0.0 - shift, 1.0 - shift);
...
void drawVector(double x1,double y1, double x2, double y2) {
	...
	else
	{
		...
		glBegin(GL_LINES);
		glVertex3d(x1, 1.0f, y1);
		glVertex3d(x2, 1.0f, y2);
		glEnd();
		glBegin(GL_TRIANGLES);
		glVertex3d(x2, 1.0f, y2);
		glVertex3d(x2 - 0.25f, 1.0f, y2 - 1.25f); //x control width of arrow, y controls the length of arrow
		glVertex3d(x2 + 0.25f, 1.0f, y2 - 1.25f);
		glEnd();
		/* restore the matrix stack and mode. */
		glPopMatrix();
		glMatrixMode(savemode);
...
void drawString(char* c) {
	...
	glRasterPos3i(0, 1.5, 1.0f);
	for (int i = 0; i< strlen(c); i++) { glutBitmapCharacter(GLUT_BITMAP_9_BY_15, c[i]); }
...
void drawStringOnScreen(char* c) {

	glMatrixMode(GL_PROJECTION);

	glPushMatrix();
	glLoadIdentity();
	gluOrtho2D(0.0, 320 * 2, 0.0, 290 * 2);
